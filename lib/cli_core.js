...
var cmdMap = {
  ...
  , comp_execute: function(optStr){
    var args = [].slice.call(arguments, 1)
        , qs = require('querystring')
        , comp_opts = qs.parse(optStr)
        , self = this;
    var fn_onsuccess = function(reply){
      log(reply);
      if (reply.statuses && (reply.statuses.length>0)) {
          self.last_cursor = reply.statuses[0].id;
      }
      setTimeout(function() {
        var status_arg_str = args.pop();
        var status_args = qs.parse(status_arg_str);
        status_args['since_id'] = self.last_cursor;
        args.push(qs.encode(status_args));
        cmdMap.execute2.apply(null, args);
      }, 10000);
    };
    log(comp_opts);
    args.unshift(fn_onsuccess);
    cmdMap.execute2.apply(null, args);
  }
  , execute2: function(onSuccess, apiStr){
 ...
 */
function apiExc(apiName, apiExtra, onSuccess){
  ...
  try{
    ...
    if (onSuccess){
        emitter.addListener("success", onSuccess);
    }
